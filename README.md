...
> 🎬 **MiaoTV** 是一个开箱即用的、跨平台的影视聚合播放器。它基于 **Next.js 14** + **Tailwind&nbsp;CSS** + **TypeScript** 构建，支持多资源搜索、在线播放、收藏同步、播放记录、本地/云端存储，让你可以随时随地畅享海量免费影视内容。
...
```bash
# 拉取预构建镜像
docker pull ghcr.io/senshinya/MiaoTV:latest

# 运行容器
# -d: 后台运行  -p: 映射端口 3000 -> 3000
docker run -d --name MiaoTV -p 3000:3000 --env PASSWORD=your_password ghcr.io/senshinya/MiaoTV:latest
```
...
services:
  MiaoTV:
    image: ghcr.io/senshinya/MiaoTV:latest
    container_name: MiaoTV
...
services:
  MiaoTV-core:
    ...
    environment:
      ...
      - REDIS_URL=redis://MiaoTV-redis:6379
...
[MIT](LICENSE) © 2025 MiaoTV & Contributors
